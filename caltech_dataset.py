...
class Caltech(VisionDataset):
    def __init__(self, root, split='train', transform=None, target_transform=None):
        ...
        self.tup_idx_class = [(tup_idx_class[i][1],tup_idx_path[i][1]) for i in range(len(idx))]
    ...
    def __getitem__(self, index):
        ...
        '''
        path = self.tup_idx_class[index][1]
        img = Image.open(open(os.path.join(self.root,path), 'rb'))
        lab = self.tup_idx_class[index][0]
        image, label = img, lab
    ...
    def __len__(self):
        ...
        '''
        length = len(self.tup_idx_class) # Provide a way to get the length (number of elements) of the dataset
        return length
